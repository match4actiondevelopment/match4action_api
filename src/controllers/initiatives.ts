...
export class InitiativesController {
  ...
  async subscribe(req: Request, res: Response): Promise<InitiativeDocument | any> {
    try {
      const userLogged = req.user;
      ...
      updateInitiative.applicants = [...updateInitiative.applicants, req.user?._id];

      const newInitiative = await User.findByIdAndUpdate(req.params.id, updateInitiative, {
        upsert: true,
        returnOriginal: false,
      });

      if (!newInitiative) {
        return res.status(404).json({ success: false, message: 'Initiative not updated.' });
      }
