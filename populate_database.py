...
def populate_ikigai_questions(db):
    """Populate ikigaiquestions collection with default questions"""
    print("\n Populating IkigaiQuestions collection...")
    ...
    try:
        ...
        db.ikigaiquestions.delete_many({})
        
        # Insert new questions
        result = db.ikigaiquestions.insert_many(questions_data)
        print(f" Inserted {len(result.inserted_ids)} questions")
        
        return [str(q_id) for q_id in result.inserted_ids]
    except Exception as e:
        print(f" Error populating ikigai questions: {e}")
        return []
...
def show_database_summary(db):
    ...
    collections = ['users', 'initiatives', 'goals', 'usertokens', 'ikigaiquestions']
